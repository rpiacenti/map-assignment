(function() {
  ...
  .component("sdCurrentThingTypeMap", {
    template: "<div id='map'></div>",
    controller: CurrentThingTypeMapController,
    bindings: {
      zoom: "@"
    }
  });

  CurrentThingTypeMapController.$inject = ["$rootScope","$scope", "$q", "$element",
  "spa-demo.geoloc.currentOrigin",
  "spa-demo.geoloc.myLocation",
  "spa-demo.geoloc.Map",
  "spa-demo.subjects.currentSubjects",
  "spa-demo.config.APP_CONFIG"];
  function CurrentThingTypeMapController($rootScope, $scope, $q, $element,
  ...
  //    vm.selectType = "";
      ...
      vm.$onInit = function() {
        console.log("CurrentThingTypeMapController");
        //CurrentOrigin.prototype.setDistance(1);
        currentOrigin.clearLocation();
      }
    ...
    function getLocation() {
      ...
      function displaySubjects(){
        //        console.log("Cache for DisplaySubjects", vm.images);
        if (!vm.map) { return; }
        vm.map.clearMarkers();
        angular.forEach(vm.imgs, function(ti){
          if(vm.currentThing.thing_subjtype != undefined && vm.currentThing.thing_subjtype == $rootScope.SelectType){
            displaySubject(ti);
          }
        });
    ...
    CurrentThingTypeMapController.prototype.updateOrigin = function() {
    ...
    CurrentThingTypeMapController.prototype.setActiveMarker = function(thing_id, image_id) {
      if (!this.map) {
        return;
      } else if (!thing_id && !image_id) {
        //  if (this.map.getCurrentMarker().title!=='origin') {
        this.map.setActiveMarker("");
        //    }
      }
      else {
    ...
    CurrentThingTypeMapController.prototype.originInfoWindow = function(location) {
    ...
    CurrentThingTypeMapController.prototype.thingInfoWindow = function(ti) {
    ...
    CurrentThingTypeMapController.prototype.imageInfoWindow = function(ti) {
